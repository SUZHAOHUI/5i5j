...
//						if(!AboutOperator.getUser().equals("A"))
						...
						// String childStr = (String) getSession().createSQLQuery(sql1).uniqueResult();
						 ...
						 if(roleName!=null&&!"".equals(roleName)&&"超级管理员".equals(roleName)){
							 ...
							 hql = "select count(*) as total from acms where acms.code in(select a.code from areacode_setid_rela a,user b where a.set_id='"+setId+"')"; 
...
//						if(!AboutOperator.getUser().equals("A"))
						...
						if(roleName!=null&&!"".equals(roleName)&&"超级管理员".equals(roleName)){
							...
							hql=" select  'C' as 'purpose',cc.call_type,count(*) as total  from temp_per_day cc  " +
									...
									"where cc.call_type=1 and uar.purpose='C' and TO_DAYS(cc.call_time)=TO_DAYS(NOW()) group by cc.call_type";
							System.out.println(hql);
						}else{
...
//							hql=" select  'C' as 'purpose',cc.call_type,count(*) as total  from cdr_call cc  left join acms a on a.acms=cc.acms left join (select * from user_acms_rela where state = 'V') uar on uar.acms_id=a.acms_id  inner join user u on u.user_id=uar.user_id and u.org_id in('"+childStr+"') left join channel c on c.channel_id=uar.channel_id  left join org o on o.org_id=u.org_id and o.set_id=u.set_id  left join call_state cs on cs.id=cc.release_cause  where cc.call_type=1 and uar.purpose='C' and TO_DAYS(cc.call_time)=TO_DAYS(NOW()) group by cc.call_type";
							hql=" select  'C' as 'purpose',cc.call_type,count(*) as total  from temp_per_day cc  " +
									" left join acms a on a.acms=cc.acms " +
									" left join (select * from user_acms_rela where state = 'V') uar on uar.acms_id=a.acms_id  " +
									" inner join user u on u.user_id=uar.user_id left join channel c on c.channel_id=uar.channel_id  " +
									" left join org o on o.org_id=u.org_id and o.set_id=u.set_id  " +
									" left join call_state cs on cs.id=cc.release_cause  where cc.call_type=1 and uar.purpose='C' and TO_DAYS(cc.call_time)=TO_DAYS(NOW())" +
